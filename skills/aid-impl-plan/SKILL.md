...
## Quick Reference - Templates & Methodology

**Templates** (Use these for every project):
- `references/templates/epic-template.md` - Epic format and content rules
- `references/templates/story-template.md` - Story format with INVEST criteria
- `references/templates/task-template.md` - Task format with full technical details
- `references/templates/contradiction-log-template.md` - Contradiction tracking

**Methodology**:
- `references/phase3-methodology.md` - Complete Phase 3 workflow
- `references/content-mapping.md` - What goes where (PRD → Epic, Tech Spec → Task)

**Key Principle**:
```
PRD Content → Epics & Stories (WHY and WHAT)
Tech Spec Content → Tasks (HOW)
```

---

## ⚠️ GOLDEN RULES OF PHASE 3
...
│                                                                          │
│   PRD has 50 pages?     → All 50 pages map to Epics/Stories             │
│   Tech Spec has 30 pages? → All 30 pages map to Tasks                   │
│   OpenAPI has 25 endpoints? → All 25 endpoints have Tasks               │
│                                                                          │
│   If a requirement exists in PRD, it MUST appear in a Story.            │
│   If a component exists in Tech Spec, it MUST appear in a Task.         │
│   If an endpoint exists in OpenAPI, it MUST have implementation Tasks.  │
│                                                                          │
│   NOTHING IS OPTIONAL. NOTHING IS "IMPLIED". NOTHING IS SKIPPED.        │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

**Why?** If it's not in Jira, it won't get built. Forgotten requirements = failed product.

**How to Verify:**
1. Create traceability matrix (PRD section → Story)
2. Create traceability matrix (Tech Spec section → Tasks)
3. Run verification agents before Phase 4
4. 100% coverage or NO PROCEED
...
┌─────────────────────────────────────────────────────────────────────────┐
│                                                                          │
│   📏 THE BIGGER THE SOURCE DOCS, THE SMALLER THE TASKS                  │
│                                                                          │
│   Source Document Size          Task Size Target                        │
│   ─────────────────────         ────────────────                        │
│   < 20 pages                    Tasks can be M (4-8 hours)              │
│   20-50 pages                   Tasks should be S (2-4 hours)           │
│   50-100 pages                  Tasks MUST be XS-S (1-4 hours)          │
│   > 100 pages                   Tasks MUST be XS (1-2 hours)            │
│                                                                          │
│   WHY? Large documents = many requirements = easy to lose track.        │
│   Smaller tasks = more checkboxes = visible progress = nothing lost.    │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```

**The Math:**
- 100-page spec with 10 large tasks = 10 things to track (LOSE DETAILS)
- 100-page spec with 100 small tasks = 100 things to track (NOTHING LOST)

**Task Splitting Rules:**

| If Task Is... | Split Into... |
|---------------|---------------|
| "Implement authentication" | Signup, Login, Logout, Refresh, Password Reset (5 tasks) |
| "Create dashboard" | Layout, Header, Sidebar, Cards, Data Fetching (5 tasks) |
| "Build API endpoints" | One task PER endpoint |
| "Set up database" | Schema, Migrations, Seed Data, Connection (4 tasks) |
...
┌─────────────────────────────────────────────────────────────────────────┐
│                                                                          │
│   🧠 DON'T HOLD IT ALL IN MEMORY - WRITE AS YOU GO                      │
│                                                                          │
│   WRONG:                                                                 │
│   Read all docs → Think about structure → Write everything at end       │
│   (Information lost in the middle!)                                     │
│                                                                          │
│   RIGHT:                                                                 │
│   Read Section 1 → Write to enriched file → Read Section 2 → Write...  │
│   (Nothing lost - progressive capture!)                                 │
│                                                                          │
│   For EACH section processed:                                           │
│   1. Read the section                                                   │
│   2. Identify Epics/Stories/Tasks                                       │
│   3. WRITE to enriched-jiras/EPIC-N.md IMMEDIATELY                     │
│   4. Move to next section                                               │
│                                                                          │
│   The enriched file grows section by section. No memory loss.           │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
**Progress Tracking:**
```
After processing Section 1:  → EPIC-1-enriched.md: 500 words
After processing Section 2:  → EPIC-1-enriched.md: 1,200 words
After processing Section 3:  → EPIC-1-enriched.md: 2,100 words
...
After processing Section N:  → EPIC-1-enriched.md: COMPLETE
```
...
┌─────────────────────────────────────────────────────────────────────────┐
│                                                                          │
│   ✓ PHASE 3 IS NOT COMPLETE UNTIL VERIFICATION PASSES                  │
│                                                                          │
│   Before moving to Phase 4 (Development):                               │
│                                                                          │
│   □ PRD Coverage Check                                                  │
│     → Every user story has a corresponding Story in Jira               │
│     → Every acceptance criterion is in a Story                         │
│     → Every success metric is traceable                                │
│                                                                          │
│   □ Tech Spec Coverage Check                                            │
│     → Every component has Tasks                                         │
│     → Every API endpoint has Tasks                                      │
│     → Every database table has Tasks                                    │
│     → Every security requirement has Tasks                              │
│                                                                          │
│   □ Completeness Check                                                  │
│     → Every Task has all 8 required fields                             │
│     → No orphan tasks (all linked to Stories)                          │
│     → No orphan stories (all linked to Epics)                          │
│                                                                          │
│   COVERAGE < 100%? → Find gaps → Add missing items → Re-verify         │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
### Purpose
Find and resolve ALL contradictions between PRD, Tech Spec, and Research BEFORE consolidating.

### CRITICAL: Contradictions FIRST, Then Consolidate

```
┌─────────────────────────────────────────────────────────────────────────┐
│                                                                          │
│   ❌ WRONG ORDER (causes hidden conflicts):                             │
│                                                                          │
│   PRD ──┐                                                                │
│         ├──► Consolidate ──► Find contradictions ──► Fix               │
│   Tech ─┘                                                                │
│                                                                          │
│   Problem: During consolidation, you make UNCONSCIOUS DECISIONS.        │
│   Contradictions get hidden in vague wording.                           │
│                                                                          │
├─────────────────────────────────────────────────────────────────────────┤
│                                                                          │
│   ✅ RIGHT ORDER (explicit resolution):                                 │
│                                                                          │
│   PRD ──┐      ┌──────────┐      ┌──────────┐      ┌─────────────┐     │
│         ├──►   │ Compare  │ ──►  │ Log &    │ ──►  │ Fix Source  │     │
│   Tech ─┘      │ Section  │      │ Resolve  │      │ Documents   │     │
│                │ by       │      │ Each     │      │             │     │
│                │ Section  │      │ Conflict │      │ THEN        │     │
│                └──────────┘      └──────────┘      │ Consolidate │     │
│                                                     └─────────────┘     │
│                                                                          │
│   Benefit: Every decision is CONSCIOUS and DOCUMENTED.                  │
│                                                                          │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| Problem with Consolidate-First | Solution with Contradictions-First |
|--------------------------------|-----------------------------------|
| Unconscious micro-decisions | Explicit, logged decisions |
| "Basically the same" assumptions | Every difference examined |
| No audit trail | Contradiction log with rationale |
| Source docs stay broken | Source docs get fixed |
| Hidden conflicts emerge later | All conflicts caught upfront |
...
## Critical Lessons Learned (Updated 2026-01)

### Lesson 1: Always Stage Enriched Content Before Jira

**Problem**: Updating Jira directly loses context and makes iteration hard.

**Solution**: Create staging files first:
```
docs/implementation-plan/enriched-jiras/
├── EPIC-1-enriched.md   # Full content before Jira
├── EPIC-2-enriched.md
└── EPIC-N-enriched.md
```

Then push to Jira via API. This allows:
- Review before committing
- Parallel verification
- Easy iteration
- Backup if Jira fails

---

### Lesson 2: Never Skip the Story Layer

**Problem**: Epic → Task creates flat structure, loses business context.

**Solution**: Always use full hierarchy:
```
EPIC (Domain)
└── STORY (User Capability)    ← CRITICAL LAYER
    └── TASK (Implementation)
```

Stories provide:
- User-facing grouping
- Business context for tasks
- Sprint planning granularity
- Acceptance testing scope

---

### Lesson 3: Spec Fixes Are NOT Developer Work

**Problem**: Creating "EPIC-0: Specification Fixes" assigns spec alignment to developers.

**Solution**:
- Fix contradictions directly in source documents (PRD, Tech Spec)
- Delete any "spec fix" tasks from Jira
- Only create tasks for IMPLEMENTATION work

---

### Lesson 4: Content Mapping Is NOT Optional

**Problem**: Random content in descriptions doesn't help developers.

**Solution**: Strict mapping rules:

| Source | Target | Content Type |
|--------|--------|--------------|
| PRD | Epic | Business value, success criteria |
| PRD | Story | User story, acceptance criteria |
| Tech Spec | Task | Implementation details |
| OpenAPI | Task | API contracts, error handling |
| Prisma | Task | Database changes |

See `references/content-mapping.md` for complete rules.

---

### Lesson 5: Verification Is Mandatory

**Problem**: Gaps discovered during development cause delays.

**Solution**: Run verification agents before marking Phase 3 complete:

1. **PRD Coverage Agent**: Every user story → Story in Jira
2. **Tech Spec Coverage Agent**: Every component → Tasks in Jira
3. **Completeness Agent**: Every task has required fields

**Gap Remediation**: Add missing tasks, then re-verify until 100%.

---

### Lesson 6: Use ADF for Rich Jira Descriptions

**Problem**: Plain text Jira descriptions hard to read.

**Solution**: Convert Markdown to ADF (Atlassian Document Format):
...
### Lesson 7: Task Naming Conventions Matter

**Problem**: Inconsistent task names hard to track.

**Solution**: Standard format:
```
E{Epic#}-T{Sequential##}: {Action Verb} {Component}
...
### Lesson 8: Always Include These in Tasks

Every task MUST have:

- [ ] **Description**: What and why (2-3 sentences)
- [ ] **Files to modify**: Exact paths
- [ ] **Code pattern**: Example or snippet
- [ ] **API contract**: If API task (method, path, request, response)
- [ ] **Error handling**: Table of error cases
- [ ] **Acceptance criteria**: Specific, testable
- [ ] **Size estimate**: XS/S/M/L/XL with hours
- [ ] **Tech Spec reference**: Section number

A task missing any of these is NOT ready for development.
