...
## Post-Task Development Pipeline (MANDATORY)

After completing ANY coding task (new features, bug fixes, refactoring, config changes), Claude MUST run the following pipeline before reporting "done" to the user:

### Step 1: Run Tests
```
Run the relevant test suite for the modified code.
- If specific test files exist for the changed code → run those first
- Then run the full test suite for the affected package/module
- ALL tests must pass before proceeding
```

### Step 2: TypeScript Compile Check
```
Run `tsc --noEmit` in the affected package.
- 0 errors required
...
- [ ] Structural correctness (references, imports, types)
...
| Config/settings changes with no code | No compilation needed |
| Research/exploration tasks | Nothing to verify |

### When Tests Fail
1. **Fix the issue** — do not report done with failing tests
2. **Re-run the full pipeline** after the fix
3. **If stuck after 3 attempts** — report the failure to the user with details
...
| `/pipeline` | pipeline-orchestrator | Start/resume automated dev pipeline |
| `/pipeline-status` | pipeline-orchestrator | Show current pipeline state |
...
| Command | Purpose |
|---------|---------|
| `/aid-pair` | Pair with Figma plugin (auth code) |
...
| 1 | `skills/aid-prd/`, `skills/cucumber-bdd/` **(Gherkin acceptance criteria)** |
...
| Role | `skills/role-developer/`, `skills/role-product-manager/`, `skills/role-tech-lead/`, `skills/role-qa-engineer/`, `skills/role-data-scientist/` |
...
| **memory-analysis-agent** | Feedback analysis & skill improvement | `/aid-improve` command |
| **code-review-agent** | Isolated code review (security, quality, docs, architecture) | Pipeline CODE_REVIEW step |
| **test-review-agent** | Isolated test quality review (coverage, mocks, independence) | Pipeline TEST_REVIEW step |
...
.aid/pipeline/       - Pipeline config (git-tracked) and runtime state (gitignored)
...
| Phase | PM | Tech Lead | Developer | QA | Data Scientist |
|-------|-----|-----------|-----------|-----|----------------|
| 0 | Market & Competitive Research | Technology & Architecture Research | Technical Spike & Research | Test Strategy Research | EDA — Data Gathering & Analysis |
| 1 | Product Requirements (PRD) | Technical Requirements Report | Feature Specification | Test Requirements & Coverage Plan | Feasibility Assessment |
| 2 | Solution Review | System Architecture Design | Technical Design | Test Architecture & Framework | Experiment Planning |
| 3 | Roadmap & Prioritization | Sprint Planning & Task Breakdown | Task Breakdown & Estimation | Test Plan & Case Design | ML Arch & Pipeline Design & Breakdown |
| 4 | Feature Validation & UAT | Code Review & Architecture Oversight | Implementation & Coding | Test Execution & Automation | Development & Validation |
| 5 | Launch & Go-to-Market | Release Engineering & Deployment | Bug Fixes & Polish | Release Certification & Sign-off | Deploy to Production & Monitoring |

Terminology mapping defined in: `.claude/references/role-phase-terminology.json`

### Skills & Agents Location

**Single source of truth** (edit here, in this AID install):
- `.claude/skills/` - All skill definitions and prompts
- `.claude/agents/` - All sub-agent definitions

`link-project` points each project's `.claude/skills` and `.claude/agents` at these
folders via symlink (macOS/Linux) or junction (Windows). There is no copy step and no
second source: update AID once and every linked project sees it immediately.

**Foundational:**
- `.claude/skills/why-driven-decision/` - **Foundational WHY skill (loads first)**

**Memory System** (References + learnings):
- `.claude/skills/memory-system/references/roles/{role}/` - Quick reference + cumulative learnings
- `.claude/skills/memory-system/references/phases/{phase}/` - Quick reference + cumulative learnings
...
Full documentation: `.claude/skills/memory-system/references/`
