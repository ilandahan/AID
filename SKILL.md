...
Interactive text link with optional dropdown indicator.

---
tags: navigation, nav, link, menu, header, footer, dropdown
notes: Header for main nav with chevron. Footer for footer links. FooterSpecial for external links.
...
tokens:
  colors: Azure Radiance (#0C8CE9), Black (#000000), Athens Gray (#F8F9FA)
  spacing: padding: 8px 16px, icon-gap: 4px
  radius: 6px
  typography: Quicksand Medium 14/24.5 (Header), 16/24 (Footer)

states:
  default: No background, standard text color
  hover: Gray background, underline, blue text (Header)

variants:
  Header: Main nav with optional chevron. 14px font.
  Footer: Footer link. 16px, dark gray text.
  FooterSpecial: External link. No padding/background.

dos:
  - Use Header for main navigation
  - Use FooterSpecial for external resources
  - Use showIcon only for dropdown items

donts:
  - Don't use Header variant in footer
  - Don't hardcode text
  - Don't mix font sizes in same nav area
...
For individual variants, use shorter descriptions:

```
[What this variant is]. [When to use it]. [Key visual characteristics].
```
...
See `descriptionParser.ts` for TypeScript parser implementation.
