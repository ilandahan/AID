...
> **⚠️ IMPORTANT: Gherkin Format Required**
>
> In AID, all acceptance criteria MUST be written in Gherkin format (Given/When/Then).
> These criteria become executable tests in Phase 4-5.
>
> Save as: `features/[feature-area]/[story-slug].feature`
>
> See `cucumber-bdd` skill for syntax reference and step definition patterns.
...
- [ ] Feature file saved to `features/[area]/` directory
- [ ] Research backing noted in feature file comments
- [ ] PRD reference (US-XXX) noted in feature file
- [ ] Appropriate @tags assigned (see Tag Guide below)

### Happy Path (@smoke @happy-path)
...
| `@error-handling` | Failure/error scenarios |
...
Before Phase 2 gate:
```bash
npm run cucumber:dry  # Validates all Gherkin syntax
```
```
