...
- `reflection-agent` (`subagent_type: general-purpose`, `model: opus`) — the quality scorer. **Resolve project→user:** load `.claude/agents/reflection-agent/AGENT-PROMPT.md` from `{{WORKSPACE}}/.claude/…` if present, else `~/.claude/…` (the engine ships user-level).
...
6. **Score the baseline:**
   ...
   c. Spawn the reflection-agent for the Tier-2 quality score:
      ...
      - Load `{{WORKSPACE}}/.claude/agents/reflection-agent/AGENT-PROMPT.md`
